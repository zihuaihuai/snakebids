...
def snakenull_generate_inputs(*args, **kwargs):
    ...
    try:
        ...
        if "Multiple path templates" in str(e):
            print(
                "[snakenull] Detected heterogeneous dataset, applying normalization..."
            )
            ...
            if "pybids_inputs" in kwargs_modified:
                ...
                for component_name, component_config in original_components:
                    if (
                        ...
                        config_no_optional["wildcards"] = [
                            w for w in wildcards if w not in ["run", "part"]
                        ]
                        component_configs.append(
                            (f"{component_name}_no_optional", config_no_optional)
                        )

                        # Pattern 2: Files with run but no part
                        config_with_run = component_config.copy()
                        config_with_run["wildcards"] = [
                            w for w in wildcards if w != "part"
                        ]
                        filters_with_run = base_filters.copy()
                        # This is challenging - we need run to be present
                        component_configs.append(
                            (f"{component_name}_with_run", config_with_run)
                        )

                        # Pattern 3: Files with all entities
                        config_full = component_config.copy()
                        component_configs.append(
                            (f"{component_name}_full", config_full)
                        )
...
class SnakenullPlugin(PluginBase):
    ...
    def add_cli_arguments(
        ...
        group = argument_groups.get_or_create_group(
            "snakenull", "Options for snakenull heterogeneous dataset handling"
        )
    ...
    def finalize_config(self, config: dict[str, Any]):
        ...
        print(
            "[snakenull] Use snakenull_generate_inputs() instead of generate_inputs() in your Snakefile"
        )
        print(
            "[snakenull] Or import: from snakebids.plugins.snakenull import snakenull_generate_inputs as generate_inputs"
        )
