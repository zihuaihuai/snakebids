...
def demonstrate_snakemake_workflow():
    ...
    snakenull_config = {
        "enabled": True,
        "label": "snakenull",
        "scope": ["subject", "session", "acquisition", "run", "part"]  # All entities we want in output
    }
    ...
    print("    output:")
    print("        \"path/to/outputs/sub-{subject}_ses-{session}_run-{run}_part-{part}_acquisition-{acquisition}_T1w_processed.nii.gz\"")
    print("    shell:")
...
if __name__ == "__main__":
    ...
    print()
    print(f"Test passed: Processed {len(normalized_inputs['t1w'].zip_lists['subject'])} heterogeneous T1w files")