    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        def mock_structural_outputs_func(wildcards, output_dir):
            ...
            if "part" in component_entities:
                return (
                    output_dir
                    + "/sub-{subject}/ses-{session}/anat/sub-{subject}_ses-{session}_acq-{acq}_run-{run}_part-{part}_space-nativepro_T1w.nii.gz"
                )
            else:
                return (
                    output_dir
                    + "/sub-{subject}/ses-{session}/anat/sub-{subject}_ses-{session}_acq-{acq}_run-{run}_space-nativepro_T1w.nii.gz"
                )
