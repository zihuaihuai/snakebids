...
def apply_snakenull_normalization(
    inputs: Dict[str, Any],
    snakenull_config: Dict[str, Any] | None = None
) -> Dict[str, Any]:
    ...
    for component_name, component in inputs.items():
        ...
        _logger.info(
            ...
            component_name, num_files, len(normalized_zip_lists)
        )
    
    return inputs


def _get_entities_in_scope(available_entities: List[str], scope: str | List[str]) -> Set[str]:
...
def _normalize_zip_lists(
    ...
    for entity in entities_to_normalize:
        if entity not in normalized:
            ...
            _logger.debug("Added missing entity '%s' with %d placeholders", entity, num_files)
        else:
...
def test_snakenull_with_t1w_dataset():
    ...
    acq_part_t1w = type('MockComponent', (), {
        'name': 't1w_acq_part', 
    ...
    acq_run_t1w = type('MockComponent', (), {
        'name': 't1w_acq_run',
    ...
    print("This demonstrates how snakenull creates unified templates from heterogeneous files\n")
    ...
    unified_component = type('MockComponent', (), {
        'name': 't1w_unified',
    ...
    print("Files with missing entities get 'snakenull' placeholders, enabling parallel processing.")
